...
import { fetchCategoryData, CategoryData } from "./activityWatchUtils";
...
interface ActivityWatchBarChartProps {
	...
	onTimeframeChange: (
		hours: number,
		bucket: string,
	) => Promise<{ data: CategoryData; prev_data: CategoryData }>;
...
export const ActivityWatchBarChart: React.FC<ActivityWatchBarChartProps> = ({
	...
	const handleTimeframeChange = async (value: string) => {
		...
		const { data: newData, prev_data: newPrevData } =
			await onTimeframeChange(hours, selectedBucket);
		setData(newData);
	...
	const handleBucketChange = async (value: string) => {
		...
		const { data: newData, prev_data: newPrevData } =
			await onTimeframeChange(hours, value);
		setData(newData);
		setPrevData(newPrevData);
	};
	...
	const chartData: ChartDataItem[] = data.categories
		...
		.map((category) => {
			const prevCategory = prevData.categories.find(
			...
			return {
				...
				previous: prevCategory
					? Number((prevCategory.duration / 3600).toFixed(2))
					: 0,
	...
	const maxDuration = Math.max(
		...chartData.map((item) => Math.max(item.current, item.previous)),
	);
	...
	const percentageChange =
		prevData.total_duration > 0
			? ((data.total_duration - prevData.total_duration) /
					prevData.total_duration) *
				100
		...
		if (active && payload && payload.length) {
			return (
				...
				>
					...
					>{`${label}`}</p>
					<p
						style={{ margin: "0", color: settings.currentBarColor }}
					>{`Current: ${payload[0].value?.toFixed(2)} hours (${(payload[0].payload as ChartDataItem).currentPercentage.toFixed(2)}%)`}</p>
					<p
						style={{
							margin: "0",
							color: settings.previousBarColor,
						}}
					>{`Previous: ${payload[1].value?.toFixed(2)} hours (${(payload[1].payload as ChartDataItem).previousPercentage.toFixed(2)}%)`}</p>
				</div>
			);
		}
		return null;
	...
	const getLegendProps = (position: LegendPosition) => {
		switch (position) {
			case "top":
				return {
	...
	return (
		...
		>
			<div
				...
				style={{ marginBottom: `${settings.controlsMarginBottom}px` }}
			>
				<h2
			...
			<div
				...
				style={{
					marginTop: `${settings.chartMarginTop}px`,
			...
			>
				<ResponsiveContainer width={settings.chartWidth} height="100%">
					...
					>
						...
						<Bar
							dataKey="current"
							fill={settings.currentBarColor}
							name="Current Period"
						/>
						<Bar
							dataKey="previous"
							fill={settings.previousBarColor}
			...
			<div
				...
				style={{
					marginTop: `${settings.footerMarginTop}px`,
					fontSize: `${settings.labelFontSize}px`,
				}}
			>
					...
					{percentageChange !== 0 ? (
						<>
							{percentageChange > 0
								? "Trending up"
								: "Trending down"}{" "}
							by {Math.abs(percentageChange).toFixed(1)}% this
							period
							{percentageChange > 0 ? (
				...
				>
					Current period: {totalHours} hours, Previous period:{" "}
